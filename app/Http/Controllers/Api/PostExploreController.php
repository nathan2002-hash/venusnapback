...
class PostExploreController extends Controller
{
    public function index(Request $request)
    {
        ...
        // 2. Get regular posts - prioritize monetized albums first
        ...
        // First, try to get 2 posts from monetized albums
        $monetizedPosts = Post::with(['postmedias', 'album.supporters'])
            ->where('status', 'active')
            ->where('visibility', 'public')
            ...
            ->inRandomOrder()
            ->take(2) // Get up to 2 monetized posts
            ->get();

        // Calculate how many more posts we need
        $remainingPostsNeeded = $postCount - $monetizedPosts->count();

        // If we need more posts, get non-monetized ones
        $nonMonetizedPosts = collect();
        if ($remainingPostsNeeded > 0) {
            $nonMonetizedPosts = Post::with(['postmedias', 'album.supporters'])
                ->where('status', 'active')
                ->where('visibility', 'public')
                ->where(function($query) {
                    $query->whereDoesntHave('album')
                        ->orWhereHas('album', function($q) {
                            $q->where('monetization_status', '!=', 'active')
                ...
                ->inRandomOrder()
                ->take($remainingPostsNeeded)
                ->get();
        }

        $posts = $monetizedPosts->merge($nonMonetizedPosts);
        ...
        Queue::push(new ProcessBatchEarningsJob([
            ...
            'monetized_posts' => $monetizedPostIds,
        ...
        return response()->json([
            ...
            'stats' => [ // Optional: for debugging
                'monetized_posts' => count($monetizedPostIds),
                'non_monetized_posts' => $posts->count() - count($monetizedPostIds),
                'ads_count' => $ads->count()
            ]
        ]);
    }
